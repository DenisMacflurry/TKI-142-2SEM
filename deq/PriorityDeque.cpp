...
bool PriorityDeque::is_valid_priority(size_t priority) const {
    return priority >= MIN_PRIORITY && priority <= MAX_PRIORITY;
}

void PriorityDeque::insert_by_priority(size_t value, size_t priority) {
...
void PriorityDeque::push_front(size_t value, size_t priority) {
    insert_by_priority(value, priority);
}
...
bool PriorityDeque::is_empty() const {
    return count == 0;
}

size_t PriorityDeque::get_size() const {
    return count;
}

std::pair<size_t, size_t> PriorityDeque::find_min_priority_element() const {
    if (count == 0)
        error_message("Очередь пуста (find_min_priority_element)");
    return data[0];
}

std::pair<size_t, size_t> PriorityDeque::find_max_priority_element() const {
    if (count == 0)
        error_message("Очередь пуста (find_max_priority_element)");
    return data[count - 1];
}

std::string PriorityDeque::to_string() const {
